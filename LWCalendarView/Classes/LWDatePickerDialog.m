...
static LWDatePickerDialog *_instance;
...
+(instancetype)initWithDate:(NSDate *)currentDate Delegate:(id<LWDatePickerDelegate>)delegate{
    return [LWDatePickerDialog initWithDate:currentDate StartDate:currentDate Delegate:delegate];
}

+(instancetype)initWithDate:(NSDate *)currentDate StartDate:(NSDate *)startDate Delegate:(id<LWDatePickerDelegate>)delegate{
    return [LWDatePickerDialog initWithDate:currentDate StartDate:startDate EndDate:startDate Delegate:delegate];
}

+(instancetype)initWithDate:(NSDate *)currentDate StartDate:(NSDate *)startDate EndDate:(NSDate *)endDate Delegate:(id<LWDatePickerDelegate>)delegate{
...
-(LWDatePickerView *)dateContainer{
    if (_dateContainer == nil) {
        _dateContainer = [[LWDatePickerView alloc] init];
        ...
        //添加约束
        NSLayoutConstraint *viewLeft = [NSLayoutConstraint constraintWithItem:_dateContainer attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeLeft multiplier:1.0 constant:LWDATEPICKERDIALOG_MARGIN_H];
        NSLayoutConstraint *viewRight = [NSLayoutConstraint constraintWithItem:_dateContainer attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeRight multiplier:1.0 constant:-LWDATEPICKERDIALOG_MARGIN_H];
        NSLayoutConstraint *viewTop = [NSLayoutConstraint constraintWithItem:_dateContainer attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeTop multiplier:1.0 constant:LWDATEPICKERDIALOG_MARGIN_V];
        NSLayoutConstraint *viewBottom = [NSLayoutConstraint constraintWithItem:_dateContainer attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-LWDATEPICKERDIALOG_MARGIN_V];
